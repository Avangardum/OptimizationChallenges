...
internal static class Iteration1
{
    ...
    private static void WriteOutput(bool[,] outputGrid)
    {
        ...
        for (int mRow = 0; mRow < iRowCount; mRow++)
        {
            for (int mColumn = 0; mColumn < iColumnCount; mColumn++) 
                iStringBuilder.Append(outputGrid[mRow, mColumn] ? LivingCellChar : DeadCellChar);
            if (mRow < iRowCount - 1) iStringBuilder.AppendLine();
        }
        File.WriteAllText(OutputPath, iStringBuilder.ToString());
    }
