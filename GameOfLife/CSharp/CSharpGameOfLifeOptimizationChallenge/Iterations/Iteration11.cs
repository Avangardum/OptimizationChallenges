...
/// <summary>
/// From Iteration10.<br/>
/// Set a fixed number of parallel jobs in RunGeneration.<br/>
/// No consistent runtime effect.
/// </summary>
internal static unsafe class Iteration11
{
    private class Grid
    {
    ...
    public static void Run()
    {
        ...
        var primaryGridCells = stackalloc bool[iRowCount * iColumnCount];
        CopyRawGridToGridCells(iInputRawGrid, primaryGridCells);
        var primaryGrid = new Grid(iRowCount, iColumnCount, primaryGridCells);
        var secondaryGridCells = stackalloc bool[iRowCount * iColumnCount];
    ...
    private static void RunGeneration(Grid inputGrid, Grid outputGrid)
    {
        ...
        const int parallelJobCount = 20;
        var rowsPerJob = iRowCount / parallelJobCount + 1;
        Parallel.For(0, parallelJobCount, job =>
        {
            int mRow = job * rowsPerJob;
            var rowMaxExclusive = Math.Min(mRow + rowsPerJob, iRowCount);
            for (; mRow < rowMaxExclusive; mRow++)
            {
                for (var mColumn = 0; mColumn < iColumnCount; mColumn++)
                {
